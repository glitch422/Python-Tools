...
def arp_poison(gw_ip, gw_mac, target_ip, target_mac):
    while True:
        ...
        time.sleep(1) #Turtelling-מנוחה בין חבילה לחבילה


def mac_grabber(gw_ip, target_ip):
    answer_gw = srp1(Ether(dst='ff:ff:ff:ff:ff:ff')/ARP(pdst=gw_ip, hwdst='ff:ff:ff:ff:ff:ff', op=1), timeour=2, verbose=0, retry=1) #retry => if not success try again (only to timeout)
